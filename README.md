...
> 当前桌面原型版本为 `0.1.2`，已完成“夜航 / Nocturne”品牌、应用图标和桌面界面实装。它仍不是可托管重要资料的正式版本。首次测试前请先阅读[数据与备份](#数据与备份)和[原型方案与边界](docs/prototype-plan.md)。
...
| 能力 | 当前范围 |
| --- | --- |
| 品牌与界面 | “夜航 / Nocturne”中英文名称、标语、月帆夜舟图标、深靛导航与宣纸编辑界面；主窗口、托盘、便签和 EXE 元数据使用同一品牌 |
...
& .\dist\Nocturne-0.1.2-portable\Nocturne.exe
```
...
- 人工/UI 冒烟通过：夜航名称、标语、窗口/便签图标、两层格式工具栏和深靛—宣纸三栏布局均已显示；快速便签可置顶呼出；浅色菜单在深色系统主题下的普通、选中、禁用文字均保持可读。测试使用隔离的 `--test-profile`，未触碰正常用户数据。
- `Nocturne.exe` 的文件说明、产品名和版本资源均为“夜航 · Nocturne”与 `0.1.2`；便携包为 `36` 个文件、`76.1 MiB`，全部二进制依赖扫描均已解析。
...
应用使用 Qt 的 `QStandardPaths::AppLocalDataLocation`。为保证从 `V0.1.1` 原位升级后仍能读取既有笔记，`V0.1.2` 只改变显示品牌和可执行文件名，内部组织名与应用名继续沿用 `FeatherNote`；因此 Windows 上的典型数据根目录仍为：
...
本轮电脑端实现已形成 `V0.1.2` 可追踪版本；下一步继续做大样本启动/切换基准、附件去重与恢复策略，并在视觉基线稳定后补充高 DPI 和窄窗口回归。手机端和网络同步仍保持暂缓。详细任务状态见[项目规划](doc/02-项目规划.md)。
