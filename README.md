...
**本次同版更新：全文 LaTeX 转换。** 点击笔记页头的“转换公式”，或在“插入”菜单／正文右键选择“转换本文全部 LaTeX”，一次处理整篇正文。支持标准分隔符、可识别的裸 LaTeX 和公式环境；代码、路径、链接与已有公式保持。转换前保留航迹，支持一次撤销，无法解析的内容保留源码并提示原因。

外部 MD / TXT 只补充公式分隔符，通过原编码和换行写回，再显示预览；外部 HTML 只转换预览。不会自动修改其他笔记。

同时修复了顶层项目刷新范围扩大的问题：刷新只读取明确对接的文档目录，导航容器不会自动成为扫描目录。旧对接库若未登记范围，需要在具体文档目录右键“设置为刷新目录”，确认真实路径后启用。本版不会自动删除旧库中已有的链接。
