...
**当前基线**：`V0.1.4` 在 `V0.1.3` 的摘要列表、按需正文、分组、多便签与一体化窗口之上，加入数据库一致性快照、附件副本、JSON 清单、每日自动限频及分类型有限保留。schema 仍为 v3，备份不改变业务数据结构。

**当前任务入口**：[DEV-004 本地数据安全与恢复](02-项目规划.md#4-任务安排)
...
文件菜单提供“立即备份本地资料”和“打开备份目录”；帮助菜单说明人工恢复边界。手动备份完成后显示实际目录。当前不在数据库仍打开时替换活动文件，恢复必须完全退出夜航后进行。
...
第一条命令默认构建并运行 CTest；第二条还生成 `dist/Nocturne-0.1.4-portable/`。包版本由 `scripts/package.ps1` 从 `CMakeLists.txt` 读取，不能另设一份硬编码版本。失败时先检查 PowerShell 输出中的 CMake、编译器、Qt 插件或 `ldd` 缺失项。
...
CTest 当前有四个入口：`NocturnePersistenceTest` 从真实 `0.1.0` 旧 schema 起步，验证 schema v3 迁移、摘要搜索、正文哈希与版本、分组、多便签独立 ID、兼容接口、待办、软删除和幂等重启；`NocturnePerformanceTest` 生成 600 篇、约 21.5 MiB 正文库并测摘要/单篇读取；`NocturneDesktopUiTest` 驱动完整 Qt 窗口，验证多便签、置顶、透明度和备份菜单入口；`NocturneBackupTest` 在线生成快照并从独立只读连接读取原笔记，同时验证嵌套附件、JSON 清单、同日限频、分类型保留以及无关目录不被清理。
