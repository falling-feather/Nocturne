...
**当前基线**：Git 提交基线为 `V0.1.2`。它具备摘要列表、按需正文、版本校验缓存、分组、正式入库便签、富文本与图片、待办、托盘、夜航品牌资源和便携打包脚本。

**当前任务入口**：[DEV-002 夜航品牌与桌面界面实装](02-项目规划.md#4-任务安排)
...
| 数据库 | `src/Database.*` | schema v2、升级迁移、摘要/正文分离、哈希版本、分组、便签和待办；不负责 UI | `Database::open()` | Qt SQL；由主窗口与便签调用 | `NocturnePersistenceTest` |
...
| --- | --- | --- | --- | --- | --- |
| 主窗口 | 程序启动、托盘打开 | 深靛左侧导航与品牌区、宣纸色编辑区、月白待办区 | 搜索、摘要列表、两层格式栏、显式重命名、分组、编辑器、待办 | 便签正文在主编辑器只读，避免双窗口互相覆盖 | `src/MainWindow.cpp` |
| 快速便签 | `Ctrl+Alt+N`、主窗口或托盘 | 深靛品牌头、月白容器与纯文本编辑区 | 品牌图标、`QTextEdit`、保存提示 | 置顶工具窗口 | `src/StickyNoteWindow.cpp` |
...
主题使用深靛 `#172238`、宣纸色 `#FBF7EE`、月金 `#F1D7A2` 和朱砂 `#A55346`。菜单与组合框弹窗是独立顶层窗口，因此 `MainWindow::applyTheme()` 必须通过 `qApp` 设置应用级调色板和样式，并显式定义普通、选中、禁用文字及分隔线；只在 `MainWindow` 上调用 `setStyleSheet()` 会重新引入 Windows 深色主题下的白底白字问题。
...
第一条命令默认构建并运行 CTest；第二条还生成 `dist/Nocturne-0.1.2-portable/`。包版本由 `scripts/package.ps1` 从 `CMakeLists.txt` 读取，不能另设一份硬编码版本。失败时先检查 PowerShell 输出中的 CMake、编译器、Qt 插件或 `ldd` 缺失项。
...
自动入口是 CMake 目标 `NocturnePersistenceTest`；`build.ps1` 默认通过 CTest 执行。它从真实 `0.1.0` 旧 schema 起步，验证 schema v2 迁移、摘要搜索、正文哈希与版本、元数据重命名、分组 CRUD/筛选/安全删除、便签迁移与兼容接口、待办、软删除和幂等重启。
