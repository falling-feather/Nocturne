...
**当前任务入口**：[DEV-008 文档与目录工作流](02-项目规划.md#4-任务安排)。工作区新增 Markdown 分级编辑、文段待办、目录树和批量导入，数据库为 schema v5；GitHub 已发布的 `v0.1.6` 仍是此前的 schema v4 版本。
...
| 数据库 | `src/Database.*` | schema v5、升级迁移、摘要/正文分离、哈希版本、目录、多便签、来源、文段待办与导入来源；不负责 UI | `Database::open()` | Qt SQL；由主窗口、便签及导入线程各自连接调用 | 持久化、性能与文档工作流回归 |
...
| 文档导入 | `src/DocumentImporter.*` | 流式处理目录，解码文本、转换 Markdown、复制本地图片、识别来源；不写回源文件 | `DocumentImporter::run()` | 工作线程独立 Database 连接；主窗口模态进度页 | 编码、重复、变更副本、取消、原文件保持与完整选择窗口链路 |
...
**当前状态**：以下功能已实装在工作区与 `dist/Nocturne-documents-preview/` 独立预览包；尚未替换 GitHub 的 `v0.1.6`。请用包内 `启动功能预览.cmd` 以隔离配置体验。工作区数据库为 schema v5，旧公开程序只支持 v4，不应使用旧程序直接打开已升级的数据库。
...
图片单击延迟一个系统双击间隔后打开原创“图片描述”窗口，留空可移除描述；双击取消待处理单击并打开原创大图预览，支持适应窗口、原始尺寸、缩小/放大和滚动。预览关闭即释放图像。图注存为图片 `ImageAltText` 和图片后带 `nocturne-caption:` 标记的普通段落；同一编辑事务支持撤销，HTML 保存重载可继续修改，Markdown 导出移除内部标记。
...
| 目录展开状态 | `QSettings` 的 `navigation/expandedFolders` | 首次展开顶层 | 记忆目录树的展开 ID | 非敏感 |
...
第一条命令默认构建并运行 CTest；第二条还生成 `dist/artifacts/Nocturne-0.2.0-portable/`。包版本由 `scripts/package.ps1` 从 `CMakeLists.txt` 读取，不能另设一份硬编码版本。失败时先检查 PowerShell 输出中的 CMake、编译器、Qt 插件或 `ldd` 缺失项。
...
python scripts/collect-runtime-notices.py --package-dir dist/artifacts/Nocturne-0.1.6-release-check --verify-sources
```
...
公开发布已验证：[v0.1.6 发布页](https://github.com/falling-feather/Nocturne/releases/tag/v0.1.6) 的三个附件均已上传，GitHub 返回的 digest 与本地文件一致；又通过匿名下载完整 ZIP 复核相同 SHA-256。发布被标记为预发布版，与当前桌面原型定位一致；源码标签固定在 `364c617`，后续发布记录提交只更新说明与文本格式，不改变该标签。
...
CTest 当前有四个入口：`NocturnePersistenceTest` 从真实 `0.1.0` 旧 schema 起步，验证 schema v5 迁移、摘要搜索、正文哈希与版本、分组、多便签、非破坏性合册、持久来源、待办、软删除和幂等重启；`NocturnePerformanceTest` 生成 600 篇、约 21.5 MiB 正文库并测摘要/单篇读取；`NocturneDesktopUiTest` 驱动完整 Qt 窗口，验证多便签、合册和快捷键：第二个原生窗口先占用候选组合，设置对话框必须报告冲突且保留旧值，释放后同一组合必须成功、同步界面并在重建主窗口后恢复；`NocturneBackupTest` 验证快照、附件、清单、限频和安全保留。
