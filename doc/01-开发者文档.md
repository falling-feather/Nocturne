...
**当前任务入口**：[DEV-006 全局捕获快捷键](02-项目规划.md#4-任务安排)
...
标题框直接承担笔记重命名，`F2` 和列表右键提供显式入口。左侧组合框筛选分组，编辑器工具栏组合框移动当前笔记；`Ctrl+Shift+G` 新建分组，`Ctrl+Shift+B` 打开“收舟入册”。合册对话框用勾选控制范围、拖动控制正文顺序，目标标题和分组明确可见，并显式提示源便签不会删除。“管理 → 唤笺快捷键”使用单段按键录入控件，实时解释不支持组合，并把真实系统占用错误留在对话框内，不会因失败关闭。列表切换前保存当前脏笔记；搜索输入约 `180 ms` 防抖。主顶栏空白区域支持原生拖动和双击最大化，边缘支持八向缩放，右上角按钮执行最小化、最大化/还原和关闭到托盘。关闭主窗口不等于退出，托盘退出才释放数据库和快捷键。
