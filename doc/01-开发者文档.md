...
**当前基线**：`V0.1.5` 在按需正文、多便签和一致性本地备份之上，加入“收舟入册”多选整理流程；所选便签按用户拖动顺序生成可继续编辑的正式笔记，源便签默认保留，来源 ID、类型、顺序和当时更新时间写入 schema v4 的独立关系表。

**当前任务入口**：[DEV-005 多便签整理与正式笔记生成](02-项目规划.md#4-任务安排)
...
标题框直接承担笔记重命名，`F2` 和列表右键提供显式入口。左侧组合框筛选分组，编辑器工具栏组合框移动当前笔记；`Ctrl+Shift+G` 新建分组，`Ctrl+Shift+B` 打开“收舟入册”。合册对话框用勾选控制范围、拖动控制正文顺序，目标标题和分组明确可见，并显式提示源便签不会删除。列表切换前保存当前脏笔记；搜索输入约 `180 ms` 防抖。主顶栏空白区域支持原生拖动和双击最大化，边缘支持八向缩放，右上角按钮执行最小化、最大化/还原和关闭到托盘。关闭主窗口不等于退出，托盘退出才释放数据库和快捷键。
...
第一条命令默认构建并运行 CTest；第二条还生成 `dist/Nocturne-0.1.5-portable/`。包版本由 `scripts/package.ps1` 从 `CMakeLists.txt` 读取，不能另设一份硬编码版本。失败时先检查 PowerShell 输出中的 CMake、编译器、Qt 插件或 `ldd` 缺失项。
...
CTest 当前有四个入口：`NocturnePersistenceTest` 从真实 `0.1.0` 旧 schema 起步，验证 schema v4 迁移、摘要搜索、正文哈希与版本、分组、多便签、非破坏性合册、持久来源、待办、软删除和幂等重启；`NocturnePerformanceTest` 生成 600 篇、约 21.5 MiB 正文库并测摘要/单篇读取；`NocturneDesktopUiTest` 驱动完整 Qt 窗口，验证多便签、置顶、透明度、备份入口和“收舟入册”对话框，并保存对话框真实渲染截图；`NocturneBackupTest` 在线生成快照并从独立只读连接读取原笔记，同时验证嵌套附件、JSON 清单、同日限频、分类型保留以及无关目录不被清理。
