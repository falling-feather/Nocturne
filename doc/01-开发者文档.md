...
本节描述 2026-09-11 的工作区实现。Release 构建、CTest 四项回归及隔离样例截图检查已通过，唯一桌面应用已原位更新并正常启动；交付证据见[已完成迭代](03-开发历史.md#21-数学公式与树状导航工作区迭代)，未分配新的公开版本。
...
`MathSupport` 是数学解析、HTML 注释转换和源码导出的唯一实现。持久化使用带源码与行内/块模式的 `nocturne-math:` 图片地址，不把公式扁平化成不可编辑附件；复制、纯文本索引、Markdown 导出均还原源码。代码块、行内代码和转义美元符不作数学转换。复制自网页的 KaTeX/MathML 若含 `application/x-tex` 注释，只取一份源码，避免重复视觉文本。旧笔记中已丢失分隔符和注释的普通文字无法无歧义恢复，可选中保留的 LaTeX 源码后右键“选区作为 LaTeX 公式”。
...
来源栏提供比较、重载与重新关联文件。目录树的“刷新”只遍历已经对接的范围，发现新文档并更新变更缓存，不重新扫描整个磁盘；被移出夜航的外部文档不会被刷新自动恢复。对接目录右键可重新关联整个目录的位置，路径批量更新在同一事务内完成，保留笔记编号和目录结构；文件位置由用户管理。
